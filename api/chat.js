...
module.exports = async function(req, res) {
    ...
    try {
          var files = selectDocs(question);
          var docs = [];
          for (var i=0; i<files.length; i++) {
                  try {
                            var r = await fetch(BASE+files[i]);
                            if (r.ok) { var t=await r.text(); docs.push('=== '+files[i]+' ===\n'+(t.length>80000?t.slice(0,80000)+'\n[truncated]':t)); }
                  } catch(e) {}
          }
          ...
          var messages = hist.slice(-6).concat([{role:'user',content:question}]);
